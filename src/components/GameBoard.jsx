...
import { RiArrowLeftCircleFill, RiArrowLeftLine, RiHeartFill } from '@remixicon/react'
...
function GameBoard() {
    // context    
    const { score, setScore, count, setCount } = useContext(scoreContext)
    ...
    return (
        <>
            <Link to={'/'} onClick={restartScores} className='iconBtn fixed left-6 top-6 '>
                <RiArrowLeftLine
                    size={24}
                    className="my-icon text-white"
                />
            </Link>

            <section className="game-board-small md:game-board">
                {colorArrangement.map((candy, index) => (
                    <motion.img whileInView={{ scale: 1 }} whileHover={{ scale: .85 }} initial={{ scale: 1.6 }} whileTap={{ scale: 1.3, border: 'none', boxShadow: 'none' }} transition={{ easings: easeInOut }}
                        loading='lazy'
                        className='w-[70px] h-[70px] p-[2px] border-1 border-amber-700'
                        src={candy}
                        key={index}
                        alt={candy}
                        data-id={index}
